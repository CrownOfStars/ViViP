...
class SegmentConsensus(torch.autograd.Function):
    ...
    def backward(self, grad_output):
        ...
        return grad_output.expand(self.shape) / float(self.shape[self.dim])
        


class TSN(nn.Module):
    def __init__(self, num_classes = 101, num_segments = 16, modality = 'RGB',
...
TSN Configurations:
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet152' == base_model:
            self.base_model = getattr(models, base_model)(True)
...
if __name__ == "__main__":
    imgs = torch.randn([8,12,3,224,224]).cuda()
    labels = torch.randn([8,101]).ge(0).float().cuda()
    model = TSN(101,12,'RGB').cuda()
